...
def topsis(matrix, weights, bencos):
    matrix = np.array(matrix, dtype=float) # define matrix from input
    weights = np.array(weights, dtype=float)  # define matrix from weights
    normalized = matrix / np.sqrt((matrix ** 2).sum(axis=0)) # normalized criteria

    # weights to criteria
    weighted = normalized * weights

    # prepare arrays for ideal and negative ideal
    ideal = np.zeros(weighted.shape[1])
    negideal = np.zeros(weighted.shape[1])

    # determining ideal and negative ideal
    for j in range(weighted.shape[1]):
        if bencos[j] == 'Benefit':
            ideal[j] = np.max(weighted[:, j])
            negideal[j] = np.min(weighted[:, j])
        else:
            ideal[j] = np.min(weighted[:, j])
            negideal[j] = np.max(weighted[:, j])

    # calculate separation measures
    sepideal = np.linalg.norm(weighted - ideal, axis=1)
    sepnegideal = np.linalg.norm(weighted - negideal, axis=1)

    # obtaining scores by relative closeness
...
if input_mode == "Input":
    ...
    if st.button("Compute"):
        try:
            ...
            # Run SAW function
            scores = topsis(matrix, weights, bencos)
